...
Spectrum VolPathIntegratorV4::Li(const RayDifferential& r, const Scene& scene, Sampler& sampler,
  ...
  while (true)
  {
    ...
    if (ray.medium)
    {
      ...
      Spectrum T_maj = SampleT_maj(
          ...
          {
            // Medium properties
            Spectrum T_maj = maj_rec.T_maj, sigma_a = maj_rec.sigma_a, sigma_s = maj_rec.sigma_s,
                     sigma_n = maj_rec.sigma_n, sigma_maj = sigma_a + sigma_s + sigma_n,
                     Le     = maj_rec.Le;
            Point3f p       = maj_rec.p;
            ...
            if (bounces <= maxDepth && !maj_rec.Le.IsBlack())
            {
              ...
              Spectrum betap = beta * T_maj / pdf, r_e = sigma_maj * T_maj / pdf;
            ...
            else if (mode == 1 /* Scatter */)
            {
              ...
              {
                // handle real scatter
                const Distribution1D* lightDistrib = lightDistribution->Lookup(p);
    ...
    if (sampleLe)
    {
      ...
      L += beta * SampleEmissionVolume(isect, scene, arena, sampler, em) / pdf_em;
    }
...
VolPathIntegratorV4* CreateVolPathIntegratorV4(const ParamSet&                      params,
                                               ...
                                               std::vector<std::shared_ptr<Medium>> emissionMediums)
{
  ...
  return new VolPathIntegratorV4(maxDepth, camera, sampler, emissionMediums, pixelBounds, sampleLe,
                                 M, rrThreshold, lightStrategy);
}

Spectrum VolPathIntegratorV4::SampleEmissionVolume(const Interaction& it, const Scene& scene,
                                                   MemoryArena& arena, Sampler& sampler,
                                                   std::shared_ptr<Medium> emission_medium) const
{
  if (!sampleLe || !emission_medium)
    return Spectrum(.0);
...
Spectrum VolPathIntegratorV4::SampleEmissionVolumeRIS(const Interaction& it, const Scene& scene,
                                                      MemoryArena& arena, Sampler& sampler,
                                                      ...
                                                      int                     M) const
{
  ...
  {
    VolumetricEmissionPoint vep;
    emission_medium->SampleEmissionPoint(sampler.Get1D(),
  ...
  if (!up.IsBlack() && !down.IsBlack())
    return up / down;
  //        return sum_w_spectrual;
  return Spectrum(.0f);
}

std::shared_ptr<Medium> VolPathIntegratorV4::SelectEmissionVolume(Float u, Float* pdf) const
{
