...
std::shared_ptr<Medium> UniformChooseOneEmissionMedium(
    Float u, const std::vector<std::shared_ptr<Medium>>& emission_mediums, Float* pdf)
{
...
Vector3f SampleDirectionTowardsEmitter(std::shared_ptr<Medium> emitter, const Interaction& it,
                                       Vector2f u2, Float* p_dir)
{
  //
}
...
VolpathIntegrator_WeightedDelta::VolpathIntegrator_WeightedDelta(
    int maxDepth, std::shared_ptr<const Camera> camera, std::shared_ptr<Sampler> sampler,
...
void VolpathIntegrator_WeightedDelta::Preprocess(const Scene& scene, Sampler& sampler)
{
  lightDistribution = CreateLightSampleDistribution(lightSampleStrategy, scene);
}

Spectrum VolpathIntegrator_WeightedDelta::Li(const RayDifferential& _ray, const Scene& scene,
    ...
    if (medium)
    {
      ...
      SampleT_maj(
          ...
          [&](MajorantSampleRecord maj_rec)
          {
            ...
            Float p_absorb  = sigma_a[channel] / (sigma_t[channel] + std::abs(sigma_n[channel]));
            Float p_scatter = sigma_s[channel] / (sigma_t[channel] + std::abs(sigma_n[channel]));
            Float p_null    = 1.f - p_absorb - p_scatter;

            int mode = SampleCollisionType(rng.UniformFloat(), p_absorb, p_scatter);
            ...
            switch (mode)
            {
              ...
              case COLLISION_SCATTER:
                ...
                if (!beta.IsBlack())
                {
                  ...
                  const Distribution1D* lightDistrib = lightDistribution->Lookup(position);
                  MediumInteraction     mi(position, -ray.d, ray.time, medium, maj_rec.phase);
...
VolpathIntegrator_WeightedDelta*
CreateVolPathIntegrator_WeightedDelta(const ParamSet& params, std::shared_ptr<Sampler> sampler,
                                      std::shared_ptr<const Camera> camera)
{
  ...
  return new VolpathIntegrator_WeightedDelta(maxDepth, camera, sampler, pixelBounds, rrThreshold,
                                             lightStrategy);
}
...
VolPathIntegrator_LineIntegration::VolPathIntegrator_LineIntegration(
    int maxDepth, std::shared_ptr<const Camera> camera, std::shared_ptr<Sampler> sampler,
    std::vector<std::shared_ptr<Medium>> emission_mediums, const Bounds2i& pixelBounds,
    bool sample_le, Float rrThreshold, const std::string& lightSampleStrategy)
    : SamplerIntegrator(camera, sampler, pixelBounds), maxDepth(maxDepth), rrThreshold(rrThreshold),
      lightSampleStrategy(lightSampleStrategy), emission_mediums(emission_mediums),
      sample_le(sample_le)
{
}

void VolPathIntegrator_LineIntegration::Preprocess(const Scene& scene, Sampler& sampler)
{
  // do nothing
}

// Do ratio tracking like line integration to gather the le along given ray
Spectrum VolPathIntegrator_LineIntegration::IntegrateLe(const Scene&           scene,
                                                        const RayDifferential& _ray,
                                                        Sampler& sampler, MemoryArena& arena) const
{
  ...
  do
  {
    ...
    if (ray.medium)
    {
      SampleT_maj(ray, t_max, rng, arena,
                  [&](MajorantSampleRecord maj_rec)
                  {
                    Spectrum T_maj     = maj_rec.T_maj;
                    Spectrum sigma_a   = maj_rec.sigma_a;
    ...
    if (!found_intersection || termiante)
      break;
...
Spectrum VolPathIntegrator_LineIntegration::Li(const RayDifferential& _ray, const Scene& scene,
...
CreateVolPathIntegrator_LineIntegration(const ParamSet& params, std::shared_ptr<Sampler> sampler,
                                        std::shared_ptr<const Camera>        camera,
                                        std::vector<std::shared_ptr<Medium>> emission_mediums)
{
  ...
  return new VolPathIntegrator_LineIntegration(maxDepth, camera, sampler, emission_mediums,
                                               pixelBounds, sample_le, rrThreshold, lightStrategy);
}
